...
from eval.metrics import compute_psnr, compute_ssim, contrast_delta, edge_slope
...
def main(**deps):
    ...
    for digit in digits:
        ...
        for i, (noisy_sample, gt_digit) in enumerate(zip(noisy_samples, gt_digits)):
            ...
            all_metrics.append(dict(psnr=compute_psnr(est, gt_digit), ssim=compute_ssim(est, gt_digit), contrast=contrast_delta(est), edge_slope=edge_slope(est, slope_mask)))
